...
func SetApiRouter(router *gin.Engine) {
	...
	anonymousRequestBodyLimit := middleware.AnonymousRequestBodyLimit()
	{
		...
		apiRouter.POST("/shejane/authorize/start", middleware.CriticalRateLimit(), middleware.DisableCache(), anonymousRequestBodyLimit, controller.StartSheJaneAuthorization)
		apiRouter.GET("/shejane/authorize/:flow_token", middleware.UserAuth(), middleware.DisableCache(), controller.GetSheJaneAuthorization)
		apiRouter.POST("/shejane/authorize/:flow_token", middleware.UserAuth(), middleware.SessionCookieOriginGuard(), middleware.CriticalRateLimit(), middleware.DisableCache(), anonymousRequestBodyLimit, controller.DecideSheJaneAuthorization)
		apiRouter.POST("/shejane/token", middleware.CriticalRateLimit(), middleware.DisableCache(), anonymousRequestBodyLimit, controller.ExchangeSheJaneAuthorization)
		apiRouter.GET("/shejane/devices", middleware.UserAuth(), middleware.DisableCache(), controller.GetSheJaneDevices)
		apiRouter.DELETE("/shejane/devices/:id", middleware.UserAuth(), middleware.SessionCookieOriginGuard(), middleware.CriticalRateLimit(), middleware.DisableCache(), controller.DeleteSheJaneDevice)
		apiRouter.POST("/shejane/telemetry/token", middleware.DisableCache(), middleware.CriticalRateLimit(), middleware.SheJaneInferenceTokenHeader(), controller.IssueSheJaneTelemetryToken)
		apiRouter.POST("/shejane/telemetry/events", middleware.DisableCache(), middleware.CriticalRateLimit(), middleware.SheJaneTelemetryAuth(), controller.IngestSheJaneTelemetryEvent)
